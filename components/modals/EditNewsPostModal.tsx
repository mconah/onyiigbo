...
interface EditNewsPostModalProps {
  newsPost: NewsPost;
  onUpdateNewsPost: (updatedNewsPost: NewsPost) => void;
  onClose: () => void;
}

const EditNewsPostModal: React.FC<EditNewsPostModalProps> = ({ newsPost, onUpdateNewsPost, onClose }) => {
  const [title, setTitle] = useState(newsPost.title);
  const [author, setAuthor] = useState(newsPost.author);
  const [category, setCategory] = useState(newsPost.category);
  const [excerpt, setExcerpt] = useState(newsPost.excerpt);
  const [content, setContent] = useState(newsPost.content); // Lexical JSON string
  const [image, setImage] = useState(newsPost.image);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
        await onUpdateNewsPost({
            ...newsPost, // Keep existing ID and date
            title,
            author,
            category,
            excerpt,
        ...
        console.error('Error updating news post in modal:', err);
        setError('Failed to update news post. Please try again.');
    } finally {
        setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
      <div className="bg-white p-8 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <h2 className="font-unica-one text-3xl font-bold text-primary-text mb-6">Edit News Post</h2>
        {error && (
